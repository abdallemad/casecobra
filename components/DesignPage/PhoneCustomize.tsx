...
function PhoneModel(
  ...
  }:
  {
    ...
    setRenderedPosition:Dispatch<SetStateAction<{
        x: number;
        y: number;
    }>>,
    setRenderDimensions:Dispatch<SetStateAction<{
        width: number;
        height: number;
    }>>
}) {
    ...
    <>
      <div 
        className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
          ...
          <div className={
            cn(
              'absolute inset-0 left-[3px] top-px right-[3px] bottom-px rounded-[32px]',`${bgColor} `
          )}/>
        </div>
        <Rnd 
          default={{
